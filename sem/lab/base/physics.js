...
export const DEFAULTS = Object.freeze({
  ...
  pressure: 1e-5, dwell: 20, resolution: 128, scanAmplitude: 20,
  ...
  flyback: 80, settleTime: 8, roughingSpeed: 1.2, roughingBase: 1e-2,
...
export function calculate(params = {}) {
  ...
  const frameTime = (p.resolution ** 2 * (p.dwell + p.settleTime) + retraceCount * p.flyback) * 1e-6;
  const electronCount = beamCurrent * 1e-9 * p.dwell * 1e-6 / e;
  ...
  const snr = noiseVariance > 0 ? detectedElectrons / Math.sqrt(noiseVariance) : 0;
  ...
  if (pmtAnodeCurrent > 100) notes.push('Illustrative PMT anode current exceeds 100 µA; a real PMT may saturate. Check its specified current and divider limits.');
  return { speed,wavelength,temperature,emission,beamCurrent,field,deflection,magneticField,
    ...
    frameTime,pmtGain,snr,dose,notes,gamma,momentum,kineticEnergy:volts,alpha,scanFieldWidth,
    sourceSize,electronCount,detectedElectrons,collectedSecondaries,collectionEfficiency,
    ...
    darkCount,noiseVariance,focusCurrent:lensFocusCurrent(p,momentum),params:p,
  };
}
...
export const EQUATIONS = [
  ...
  {id:'temperature',title:'Filament heat balance',formula:'Pₕ = εσAᵣ(T⁴ − T₀⁴) + Gₜ(T − T₀)',
    ...
    assumptions:'Assumed 2.2 mm² radiating area, ε = 0.35 and Gₜ = 0.25 mW/K. No warm-up transient, temperature-dependent resistance or evaporation. Power is not a gun-specific heater setting.',sources:[S.heat]},
  {id:'emission',title:'Thermionic emission',formula:'J = AᴿT² exp(−φ/kᴮT); Iₑ = J Aₑ',
    substitution:(p,d)=>`J = 6 × 10⁵ × ${f(d.temperature,4)}² × exp[−4.5 eV/(kᴮ × T)]`,
    result:(p,d)=>`${f(d.emission)} µA emitted`,
    derivation:'Integrating the thermal electron flux whose normal energy exceeds the work function yields the T² exp(−φ/kᴮT) dependence. Multiply current density by the effective emitting area to obtain current.',
    assumptions:'Illustrative tungsten Aᴿ = 6 × 10⁵ A·m⁻²·K⁻², φ = 4.5 eV, emitting area 10⁻¹⁰ m². Surface state, field lowering and space-charge limitation are omitted.',sources:[S.emission]},
  {id:'beamCurrent',title:'Aperture & Wehnelt transmission',formula:'Iᵦ = Iₑ[1 − exp(−rₐ²/2s²)] · w · P₀',
    substitution:(p,d)=>`${f(d.emission)} µA × ${f(d.apertureTransmission)} aperture × ${f(d.extraction)} extraction × ${f(d.survival)} transmission`,
    ...
    assumptions:'Gaussian beam s = 500 µm. w = clamp[(Vw + 300)/300, 0, 1]² is an explicitly invented teaching surrogate, not a cited universal Wehnelt law. JEOL supports the role of bias, not this transfer curve.',sources:[S.gun]},
  ...
  {id:'magneticField',title:'Lens-coil magnetic field',formula:'Bz ≈ μ₀ N I / ℓ',
    ...
    assumptions:'Air-core solenoid approximation with 452 turns and 20 mm length. Actual pole pieces, permeability, finite-coil fringe fields and saturation require a measured or solved field map.',sources:[S.solenoid]},
  ...
  {id:'spotSize',title:'Probe-size budget',formula:'d² ≈ d₀² + (0.61λ/α)² + (Csα³/2)² + (CcαΔE/E)² + d𝒇²',
    ...
    assumptions:'Assumed source 20 µm/100 demagnification, Cs = 50 mm, Cc = 20 mm, ΔE = 1 eV. An approximate diameter convention, not a predicted SEM resolution; specimen scattering, drift and astigmatism are absent.',sources:[S.probe]},
  ...
  {id:'pressure',title:'Pump-down & gas load',formula:'V dp/dt = Q − Seff(p − pb); p∞ = pb + Q/Seff',
    ...
    assumptions:'Q includes leaks/outgassing. Piecewise pump model: roughing base 10⁻² mbar, turbo base 10⁻⁷ mbar, crossover 0.1 mbar with backing pump on. These are teaching assumptions, not pump specifications; real desorption takes longer.',sources:[S.throughput]},
  {id:'meanFreePath',title:'Neutral-gas mean free path',formula:'λg = kᴮTg/(√2 π d² p)',
    ...
    assumptions:'Air-like neutral gas with assumed 0.37 nm collision diameter at room temperature. This is not the electron mean free path.',sources:[S.gas]},
  {id:'survival',title:'Electron transmission through gas',formula:'n = p/(kᴮTg); λe = 1/(nσe); P₀ = exp(−Lcol/λe)',
    ...
    assumptions:'Illustrative constant σe = 10⁻²⁰ m², not a value from NIST. Real cross sections depend on beam energy and gas species, as the linked NIST database demonstrates; this proxy is not calibrated.',sources:[S.scattering,S.gas]},
  {id:'frameTime',title:'Minimum scan timing',formula:'tmin = N²(tdwell + tsettle) + R tflyback',
    ...
    assumptions:'Default 8 µs settling and 80 µs retrace. Excludes SPI transfers, ADC conversion, software and serial transport. Custom paths use N² timing slots including blank points; actual arbitrary-program duration requires measurement.',sources:[S.detector]},
  {id:'dose',title:'Electron count & fluence',formula:'Ne = Iᵦ tdwell/e; D = Ne/(FOV/N)²',
    substitution:(p,d)=>`${f(d.beamCurrent)} nA × ${f(p.dwell)} µs / e; pixel pitch ${f(d.pixelPitch)} µm`,
    ...
    assumptions:'One uniformly sampled square frame. Revisited positions accumulate additional dose; arbitrary custom trajectories are not necessarily uniform. Zero scan width has undefined areal fluence.',sources:[S.electric]},
  {id:'collection',title:'Secondary-electron signal chain',formula:'Npe = Ne · δSE · ηc · Yγ · ηopt · QE',
    ...
    assumptions:'Synthetic δSE = 0.2, Yγ = 20, ηopt = 0.1, QE = 0.2. ηc = 0.05 + 0.65(1 − exp(−max(Vc,0)/100)) is an invented collection trend. The collector cage and scintillator bias are separate from PMT bias; PMT detects light.',sources:[S.detector]},
  {id:'pmtGain',title:'Photomultiplier gain',formula:'G = δdynⁿ ∝ VPMTᵃⁿ; G = G₀(VPMT/800 V)⁷',
    substitution:(p,d)=>`${f(d.params.pmtReferenceGain)} × (${f(p.pmtVoltage)}/800)⁷`,
    result:(p,d)=>`${f(d.pmtGain)}× gain · ${f(d.pmtAnodeCurrent)} µA mean anode current`,
    derivation:'Each dynode multiplies the incident electrons by δdyn. Cascading n stages gives δdynⁿ. With δdyn proportional to interstage voltage to a power a, a fixed divider gives G proportional to VPMTᵃⁿ. Anode current is eGNpe/tdwell.',
    assumptions:'Illustrative calibration: G₀ = 10⁵ at 800 V, exponent 7. Real tube/divider gain, permitted voltage and current require its datasheet. Saturation is flagged but not modeled.',sources:[S.pmt]},
  {id:'snr',title:'Signal-to-noise estimate',formula:'SNR = Npe / √[F{Npe(1 + q) + Nd} + (σread/G)²]',
    substitution:(p,d)=>`${f(d.detectedElectrons)} / √(${f(d.noiseVariance)}); q = Yγ ηopt QE`,
    ...
    assumptions:'Illustrative F = 1.4, dark rate 100/s, output read noise 2 × 10⁶ electrons/sample. Counts, efficiencies and independent fluctuations are assumed. PMT gain reduces read-noise impact; it does not remove shot noise.',sources:[S.noise]},
];
